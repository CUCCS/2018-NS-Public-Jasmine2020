...
## 3.实验过程
- 设备环境准备
扫描机（充当客户端角色）kali系统 IP地址： 10.0.2.15
被扫描机（充当服务器角色）kali系统 IP地址：10.0.2.6
<br>
- 端口环境准备
...
```
![](https://i.imgur.com/7nTfTPv.png)<br>
由于kali自带的python版本是2.7，因此可以使用上述指令直接开启端口<br>
![](https://i.imgur.com/vE5m1H7.png)<br>
在python3中，该SimpleHTTPServer模块已合并到http.server，因此python3中的同样操作的指令语句为
```
python -m http.server 80
```
用nmap检查端口的情况，验证此时80端口处于打开状态，8080端口处于关闭状态
![](https://i.imgur.com/ICRO0lm.png)<br>
对于udp端口的开放状态，用以下几行代码来实现
...
s.bind((addr,port))
```
用nmap检查端口的情况，验证此时67端口处于关闭状态,然而此时nmap扫描53端口的结果为open|filter
![](https://i.imgur.com/1HB6ZJ5.png)
于是采用下面的额方法重新试图打开53端口
...
- 实验内容
	- TCP connect scan
扫描机10.0.2.15上用python运行写好的tcp连接扫描代码，同时，被扫描机10.0.2.6上用tcpdump监听对应的网卡端口
![](https://i.imgur.com/PtJP1Ru.png)<br>
![](https://i.imgur.com/DRhH15m.png)
可由监听结果中看出，与开放的80端口建立连接时，两台主机间有明显的三次交互过程
1.10.0.2.15->10.0.2.6 [S] SYN
2.10.0.2.6->10.0.2.15 [S.] SYN,ACK
3.10.0.2.15->10.0.2.6 [R.] RST,ACK
然而与关闭的8080端口连接时，只有两次交互过程
1.10.0.2.15->10.0.2.6 [S] SYN
2.10.0.2.6->10.0.2.15 [R.] RST,ACK
为了更清楚的观察到包中Flag标志位的特点，我们抓包后用wireshark图形界面来分析观察
...
<br>
	- TCP stealth scan
扫描机10.0.2.15上用python运行写好的tcp隐形扫描代码，同时，被扫描机上用tcpdump监听对应的网卡端口
![](https://i.imgur.com/urKEyd4.png)<br>
![](https://i.imgur.com/JzXe8CI.png)
可由监听结果中看出，与开放的80端口建立连接时，两台主机间有明显的三次交互过程
1.10.0.2.15->10.0.2.6 [S] SYN
2.10.0.2.6->10.0.2.15 [S.] SYN,ACK
3.10.0.2.15->10.0.2.6 [R] RST
然而与关闭的8080端口连接时，只有两次交互过程
1.10.0.2.15->10.0.2.6 [S] SYN
2.10.0.2.6->10.0.2.15 [R.] RST,ACK
为了更清楚的观察到包中Flag标志位的特点，我们抓包后用wireshark图形界面来分析观察
![](https://i.imgur.com/7ITfqr0.png)![](https://i.imgur.com/EUmnuGt.png)
![](https://i.imgur.com/UHTwcLC.png)![](https://i.imgur.com/MNCEaCe.png)
<br>
	- TCP XMAS scan
扫描机10.0.2.15上用python运行写好的tcp XMAS扫描代码，同时，被扫描机10.0.2.6上用tcpdump监听对应的网卡端口
![](https://i.imgur.com/tMVyF3O.png)<br>
![](https://i.imgur.com/zmK4mSq.png)
可由监听结果中看出，与开放的80端口建立连接时，只有扫描机给被扫描机的发包过程
1.10.0.2.15->10.0.2.6 [FPU] PSH，FIN，URG
然而与关闭的8080端口连接时，有两次交互过程
1.10.0.2.15->10.0.2.6 [FPU] PSH、FIN和URG
2.10.0.2.6->10.0.2.15 [R.] RST,ACK
为了更清楚的观察到包中Flag标志位的特点，我们抓包后用wireshark图形界面来分析观察
![](https://i.imgur.com/aiiUopY.png)![](https://i.imgur.com/gTnUeJV.png)
![](https://i.imgur.com/NEXQvgE.png)![](https://i.imgur.com/06fhY9c.png)
<br>
	- UDP scan
扫描机10.0.2.15上用python运行写好的udp扫描代码，同时，被扫描机10.0.2.6上用tcpdump监听对应的网卡端口（方法一打开53端口）
![](https://i.imgur.com/imcsgDj.png)<br>
![](https://i.imgur.com/GgA4sXX.png)
可知，此刻scapy的扫描结果显示53端口为open|filter状态，说明被扫描机没有发送任何响应包给扫描机，监听的结果与预想中一致
只有10.0.2.15发送给10.0.2.6的UDP包，没有任何响应包
对于关闭的67端口，可以观察到两台主机间有两次交互
1.10.0.2.15->10.0.2.6 [|domain] UDP
2.10.0.2.6->10.0.2.15 ICMP 
为了更清楚的观察到包中Flag标志位的特点，我们抓包后用wireshark图形界面来分析观察
![](https://i.imgur.com/CUhmgjj.png)![](https://i.imgur.com/cZdVMXF.png)
![](https://i.imgur.com/zqona3y.png)![](https://i.imgur.com/kapaeQX.png)
<br>
采用第二种方式打卡53端口后对该端口进行重复实验
![](https://i.imgur.com/oreX7UG.png)<br>
![](https://i.imgur.com/Jtpxfaq.png)
这次可以观察到两台主机之间的交互
1.10.0.2.15->10.0.2.6 [UDP]
2.10.0.2.6->10.0.2.15 [UDP]
